...
# 2 3 3 
# 30 23 23 # 第一個伺服器傳送到第1/2/3個城市的流量
# 5 25 3 # 第二個伺服器傳送到第1/2/3個城市的流量
# 0 0 # 方案一：第一個伺服器放在第一個城市，第二個伺服器放在第一個城市
# 0 1 # 方案二：第一個伺服器放在第一個城市，第二個伺服器放在第二個城市
# 0 2 # 方案三：第一個伺服器放在第一個城市，第二個伺服器放在第三個城市
...
for i in range(k):
    ...
    for server_idx in range(n): # 遊歷每個伺服器(0~n-1)
        server_city = c[server_idx] # 取得該伺服器的擺放城市(在哪個城市運行伺服器) 假設server_idx=0, c[0]如果等於1就是第1個server放在第二個城市運行
