...
# 2. 數列和公式解法
# Sn = n/2*(2a+n-1) (d=1) #Sn: 數列和, n: 連續數列長度, a: 首項
# 2Sn = n*(2a+n-1)
# 第一個條件n的範圍: a>0且因數對關係 所以 2<=n<= sqrt(2Sn)
# 第一個條件n: 2Sn/n要能整除 -> 2*Sn%n == 0
# 第二個條件a = (2Sn/n-(n-1))/2 要能整除 -> (2*Sn/n - n + 1)%2 == 0
    ...
    for n in range(2, int(math.sqrt(2 * Sn)) + 1):
        if (2 * Sn) % n == 0:  # 檢查 2Sn 是否能被 n 整除
            if (2 * Sn / n - n + 1) % 2 == 0:  # 確保 (2Sn/n - n + 1) 是偶數，才能確保 a 是整數
                ...
                if a <= 0:  # 如果 a 小於等於 0，則不可能形成有效的數列，提前中斷
                    break
                else:
                    end = a + n - 1  # 計算數列的結尾數字
