...
# 第一個條件n 的範圍: 由上面的式子可得2<=n<=sqrt(2Sn), n一定至少是2 (2a+n-1一定大於n->sqrt(2Sn)>=n)
# 第一個條件: 2Sn/n要能被整除-> 2*Sn%n == 0 (n是整數, 2a也是整數, (2a+n-1), 大家都是整數)
...
for s in sys.stdin:
    ...
    for n in range(2, int(math.sqrt(2 * Sn)) + 1): # 因為range會漏掉所以要+1
        if (2 * Sn) % n == 0:  # 檢查 2Sn 是否能被 n 整除
            if (2 * Sn / n - n + 1) % 2 == 0:  # 確保 (2Sn/n - n + 1) 確保 a 是整數
